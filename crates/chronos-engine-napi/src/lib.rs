...
pub fn diff(a_iso: String, b_iso: String, unit: String) -> Result<f64> {
    let result = catch_unwind(|| -> std::result::Result<f64, String> {
        chronos_engine::diff(&a_iso, &b_iso, &unit)
    });
    match result {
...
pub fn to_zone(utc_iso: String, zone: String) -> Result<ZonedOutputNapi> {
    ...
    match result {
        Ok(Ok(z)) => Ok(ZonedOutputNapi {
            iso: z.iso,
            offset_minutes: z.offset_minutes,
